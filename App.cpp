...
App::App()
    ...
    chickenKillCount(0),
    level3ChickensSpawned(false),
    level3BossSpawned(false),
    level3ChickenKillCount(0)
{
...
void App::spawnLevel3Chickens() {
    chickens.clear();
    const int rows = 3;
    const int cols = 6;
    const float startX = 150;
    const float startY = 100;
    const float spacingX = 100;
    const float spacingY = 80;
    ...
    for (int row = 0; row < rows; ++row) {
        for (int col = 0; col < cols; ++col) {
            ...
            // Bắt đầu từ trên màn hình và bay xuống
            chicken->set_rect_cordinate(targetX, -100 - row * 50);
            chicken->setTargetPosition(targetX, targetY); // Cần thêm hàm này trong Chicken

            chicken->set_rect_width_and_height(75, 68);
            chicken->set_is_on_screen(true);
            chicken->set_health(2); // Có thể tăng máu cho khó hơn
            chicken->set_speed(2);
    ...
    std::cout << "Spawned " << chickens.size() << " chickens in formation for level 3" << std::endl;
}
void App::spawnLevel3Boss() {
    bosses.clear();
...
void App::update(float dt) {
    ...
    case GameState::PLAYING: {
        ...
        if (currentLevel == 1) {
            ...
            // Phase 1: Spawn gà một lần duy nhất
            if (!level3ChickensSpawned) {
                spawnLevel3Chickens();
                level3ChickensSpawned = true;
            }

            // Xử lý gà vs player
            if (!level3BossSpawned) {
                processChickenVsPlayer(dt);

                // Kiểm tra nếu tất cả gà đã chết
                ...
                if (allChickensDead && !level3BossSpawned) {
                    // Clear gà còn sót lại
                    for (auto* c : chickens) delete c;
                    chickens.clear();
                // Spawn boss
                spawnLevel3Boss();
                level3BossSpawned = true;

                if (level3BossSpawned) {
                    processBossVsPlayer(dt);

                    // Kiểm tra boss chết để kết thúc level
                    if (allBossDead() && player.get_health() > 0) {
                        for (auto* b : bosses) delete b;
                        bosses.clear();
                        //app.close(); ket thuc game
                        break;
                    }
                }
...
void App::processBossVsPlayer(float dt) {
    ...
    for (auto* boss : bosses) {
        ...
        if (boss->get_health() <= 0) {
            ...
            Explosion* exp = new Explosion;
            exp->load_animation_sprite("res/image/explosion.png");
            exp->set_clips(); // thiet lap frame cat cho animation
